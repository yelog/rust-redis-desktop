...
pub fn App() -> Element {
    ...
    on_save: move |config: ConnectionConfig| {
                                ...
                                spawn(async move {
                                    tracing::info!("Saving connection: {} ({})", name, id);

                                    // Save to storage first (always succeeds)
                                    if let Some(storage) = config_storage.read().as_ref() {
                                        if let Err(e) = storage.save_connection(config.clone()) {
                                            tracing::error!("Failed to save connection: {}", e);
                                        } else {
                                            tracing::info!("Connection saved to storage");
                                        }
                                    }

                                    // Try to connect (may fail if Redis not running)
                                    if let Err(e) = connection_manager.read().add_connection(config.clone()).await {
                                        tracing::warn!("Failed to connect: {} (config still saved)", e);
                                    } else {
                                    ...
                                    if let Some(storage) = config_storage.read().as_ref() {
                                        match storage.load_connections() {
                                            Ok(saved) => {
                                                ...
                                                tracing::info!("Loaded {} connections", list.len());
